...
const configUrl = require('./url')
const packageInfo = require('../../../package.json')
const appRoot = path.resolve(__dirname, '../../../')
const corePath = path.resolve(appRoot, 'core/')
const testingEnvs = ['testing', 'testing-mysql', 'testing-pg']
let defaultConfig = {}
let knexInstance
...
ConfigManager.prototype.set = function (config) {
  let localPath = ''
  const defaultStorage = 'local-file-store'
  let contentPath
  let activeStorage
  let storagePath
  let subdir
...
ConfigManager.prototype.writeFile = function () {
  const configPath = this._config.paths.config
  const configExamplePath = this._config.paths.configExample

  return new Promise(function (resolve, reject) {
    fs.stat(configExamplePath, function checkTemplate (err) {
      const templateExists = !(err)
      let read
      let write
      let error
...
ConfigManager.prototype.validate = function () {
  const envVal = process.env.NODE_ENV || undefined
  let hasHostAndPort
  let hasSocket
  let config
  let parsedUrl
